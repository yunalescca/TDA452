...
guessing lo hi =
    do putStr "Is it "
       ...
       case ans of
           "yes" -> return ()
           "higher" -> guessing guess hi -- must put these in the right order
...
-- each element generated by g
-- A predefined function vectorOf does the same thing

-- When we return a monad, we must make sure that we actually return a monad
-- if we don't get any values from the function, we need the return call
listOf' :: Integer -> Gen a -> Gen [a] -- e.g. sample $ listOf' 2 (elements [1..10])
listOf' 0 _ = return [] -- will "convert" [a] to Gen [a]
listOf' n g = 
    do x  <- g -- use generator to get an x 
       xs <- listOf' (n - 1) g -- save list of element -- will produce a list (xs :: Gen [a])
       return (x : xs) -- add x to list 



-- * B) 
-- Now use listOf to write a generator that generates pairs of 
-- lists of the same, random, length.


genPairs :: Gen a -> Gen ([a],[a])
genPairs g = 
    do l <- (arbitrary :: Gen Integer)
       a <- listOf' (abs l) g
       b <- listOf' (abs l) g
       return (a, b)
